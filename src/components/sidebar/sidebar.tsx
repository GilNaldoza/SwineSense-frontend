        ...
        {items.map((item) => {
          ...
          return (
            <button
              key={item.label}
              onClick={() =>
                item.nav ? navigate(item.path!) : setSection(item.key!)
              }
              className={`
            ...
            >
              <span
                className={`${isRestricted ? (isArchive ? "text-red-600" : "text-purple-600") : isActive ? "text-blue-600" : "text-gray-500"} ${item.nav ? "text-green-600" : ""}`}
              >
                {item.icon}
              </span>
