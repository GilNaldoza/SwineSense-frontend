...
import { User, Bell, Menu, AlertTriangle, Activity } from "lucide-react";
...
const Header = ({ onToggleSidebar }: HeaderProps) => {
  ...
  const [alerts, setAlerts] = useState<{ atRisk: number; sick: number }>({
    atRisk: 0,
    sick: 0,
  });
  ...
  useEffect(() => {
    const fetchAlerts = async () => {
      try {
        const stats = await getPigDashboardStats();
        setAlerts({
  ...
  const totalAlerts = alerts.atRisk + alerts.sick;
