  ...
  switch (option) {
    ...
    case "time_desc":
      return copy.sort((a, b) => {
        const [ah, am] = String(a.logTime || "0:0")
          .split(":")
          .map((n) => Number(n) || 0);
        const [bh, bm] = String(b.logTime || "0:0")
          .split(":")
          .map((n) => Number(n) || 0);
    ...
    case "name_az":
      return copy.sort((a, b) => {
        const al = String(a.lastName || "").localeCompare(
          String(b.lastName || ""),
        );
        if (al !== 0) return al;
    ...
    case "name_za":
      return copy.sort((a, b) => {
        const al = String(b.lastName || "").localeCompare(
          String(a.lastName || ""),
        );
        if (al !== 0) return al;
        return String(b.firstName || "").localeCompare(
          String(a.firstName || ""),
        );
      });
    default:
  ...
  const userType =
    section === "Students"
      ...
      : section === "Faculties"
        ...
        : "all";
  const selectedYear =
    yearCtx?.yearLevel && yearCtx.yearLevel !== "all"
      ? String(yearCtx.yearLevel)
      : undefined;
    ...
    <div>
      {isError ? (
        <div className="mb-2 flex flex-col items-center">
          ...
          <div className="mt-2 text-sm text-destructive">
            Error: {String((errorObj as Error)?.message ?? errorObj)}
          </div>
        </div>
      ) : null}
      {isLoading ? (
      ...
      <ReusableTable
        ...
        onEdit={(row) => {
          ...
          const userType = (role === "faculty" ? "faculty" : "student") as
            | "student"
            | "faculty";
